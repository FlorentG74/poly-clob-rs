...
use crate::controller::DATA_API;
use crate::controller::POSITIONS;
use reqwest::Method;
...
impl WebserviceRequest {
    pub fn new_positions_ws_request(user: &str) -> Self {
        ...
        args.insert(String::from("user"), user.to_string());
...
impl WebserviceResponse for PositionsResponse {
    async fn store(&self) {
        //let db_connection_pool = get_db_connection_pool();

        //todo!("Can't store positions");
    }
...
impl Position {
    pub fn position_vec_to_map(positions: Vec<Self>) -> HashMap<String, Self> {
